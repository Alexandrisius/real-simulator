...
import { getScene, listAttributes, listClothingSlots } from "@/db/queries";
import { verifyForKeyObservers } from "./knowledge";
import { patchState } from "./knowledge";
...
export const WEAR_SPEC: ToolSpec = {
  name: WEAR_TOOL_NAME,
  description: "Надеть обратно последнее снятое со слота (top / bottom / underwear…).",
  parameters: {
    type: "object",
    properties: {
      slot: { type: "string", description: "Слот одежды: top, bottom, underwear…" },
...
/** Снятие: место -> порядок слоёв -> обнажение слота и верификация скрытого. */
export function executeUndress(input: {
  ...
  const item = wornItem(actor, slot.slot);
  ...
  if (slot.undressPlaces.length > 0 && !slot.undressPlaces.includes(place)) {
    return fail(
  ...
  const outer = slots.filter(
    (s) => s.slot !== slot.slot && s.layer < slot.layer && wornItem(actor, s.slot) != null
  );
  if (outer.length > 0) {
    return fail(
      `Сначала сними верхнее: мешает ${outer.map((s) => `${s.slot} (${wornItem(actor, s.slot)})`).join(", ")}.`
    );
  }

  // Снятие: слот пустеет, предмет — в руках (carried)
  patchState(actor.id, {
  ...
  const fresh = { ...actor, state: { ...actor.state, [WORN_PREFIX + slot.slot]: "" } };
  ...
  for (const def of defs) {
    ...
    const stillCovered = def.coveredBy.some((cs) => wornItem(fresh, cs) != null);
    if (stillCovered) continue;
    const trueValue = fresh.state[def.key];
  ...
  return {
    ...
    audience: "all",
    stateChanges: [
      { characterId: actor.id, key: WORN_PREFIX + slot.slot, value: "" },
      { characterId: actor.id, key: CARRIED_PREFIX + slot.slot, value: item },
    ],
  };
}
...
export function executeWear(input: {
  ...
  if (wornItem(actor, slot.slot)) {
    return fail(`В слоте ${slot.slot} уже надето: ${wornItem(actor, slot.slot)}.`);
  }
  const carried = actor.state[CARRIED_PREFIX + slot.slot];
  ...
  return {
    ok: true,
    result: `Надето: ${carried} (слот ${slot.slot}).`,
    observation: `${actor.name} надевает: ${carried}`,
    audience: "all",
    stateChanges: [
      { characterId: actor.id, key: WORN_PREFIX + slot.slot, value: carried },
      { characterId: actor.id, key: CARRIED_PREFIX + slot.slot, value: "" },
    ],
  };
}
